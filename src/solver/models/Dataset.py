...
def get_one_dgl_graph(g):
    ...
    # Set node features directly from node types list
    dgl_graph.ndata["feat"] = torch.tensor(g["node_types"], dtype=torch.int64)

    # Set edge weights directly as a tensor of ones
    dgl_graph.edata["weight"] = torch.ones(len(g["edges"]), dtype=torch.float32)
...
def get_edge_src_and_dst_list(edges):
    ...
    return torch.from_numpy(pd.DataFrame(edges_src).to_numpy().flatten()), torch.from_numpy(
        pd.DataFrame(edges_dst).to_numpy().flatten())
...
class WordEquationDatasetMultiClassification(DGLDataset):
    def __init__(self, graph_folder="", data_fold="train", node_type=3, graphs_from_memory: List[Dict] = [],
                 label_size=3):
        self._data_fold = data_fold
        self._graph_folder = graph_folder
        self._graphs_from_memory = graphs_from_memory
...
class WordEquationDatasetMultiClassificationLazy(WordEquationDatasetMultiClassification):
    def __init__(self, graph_folder="", data_fold="train", node_type=3, graphs_from_memory=[], label_size=3):
        ...
        super().__init__(graph_folder=graph_folder, data_fold=data_fold, node_type=node_type,
                         graphs_from_memory=graphs_from_memory, label_size=label_size)
        ...
        with zipfile.ZipFile(zip_file, 'r') as zip_file_content:
            for graph_file in tqdm(zip_file_content.namelist(), desc="Processing zip_file_content.namelist()"):
                if fnmatch.fnmatch(graph_file, "*.graph.json"):
                    ...
                    if self._label_size + 1 == len(
                            split_graphs):  # self._label_size+1 because split_graphs has a additional file_path key
