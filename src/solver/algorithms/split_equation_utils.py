...
def order_equations_unsatcore(f: Formula, category_call=0) -> (Formula, int):
    ...
    return order_equations_with_unsatcore_and_fixed_condition(f, condition, empty_else_func, unsatcore_eq_score_func,
                                                              category_call)
...
def order_equations_unsatcore_category(f: Formula, category_call=0) -> (Formula, int):
    condition = len([e for e in f.eq_list if e in f.unsat_core]) > 0
    unsatcore_eq_score_func = unsatcore_eq_score_random
    return order_equations_with_unsatcore_and_fixed_condition(f, condition, order_equations_category,
                                                              unsatcore_eq_score_func, category_call)
...
def order_equations_with_unsatcore_and_fixed_condition(f: Formula, condition, else_func, unsatcore_eq_score_func,
                                                       category_call=0) -> (Formula, int):
    if f.unsat_core == []:
        ...
        if condition:  # if there is at least one unsat core equation
            ...
            eq_list_with_unsat_core_score: List[Tuple[Equation, int]] = [
                (e, unsatcore_eq_score_func(e.term_length, f.total_eq_size)) if e in f.unsat_core else (
                    e, f.total_eq_size) for e
                in f.eq_list]
...
def apply_rules_prefix(eq: Equation, f: Formula, fresh_variable_counter) -> Tuple[
...
def apply_rules_suffix(eq: Equation, f: Formula, fresh_variable_counter) -> Tuple[
    ...
    # both sides are not empty
        ...
        # both side are differernt variables, R_{9} suffix version in paper
            ...
            rule_list: List[Callable] = [_two_variables_branch_3_suffix, _two_variables_branch_1_suffix,
                                         _two_variables_branch_2_suffix]
            children, fresh_variable_counter = _get_split_children(eq, f, rule_list, fresh_variable_counter)
...
def _get_split_children(eq: Equation, f: Formula, rule_list: List[Callable], fresh_variable_counter: int) -> Tuple[
    List[Tuple[Equation, Formula, str]], int]:
    children: List[Tuple[Equation, Formula, str]] = []
...
def _left_variable_right_terminal_branch_1_prefix(eq: Equation, current_formula: Formula,
                                                  fresh_variable_counter: int) -> \
        Tuple[
...
def _left_variable_right_terminal_branch_1_suffix(eq: Equation, current_formula: Formula,
                                                  fresh_variable_counter: int) -> Tuple[Equation, Formula, int, str]:
    '''
...
def _left_variable_right_terminal_branch_2_prefix(eq: Equation, current_formula: Formula,
                                                  fresh_variable_counter: int) -> \
        Tuple[
            Equation, Formula, int, str]:
    '''
...
def _left_variable_right_terminal_branch_2_suffix(eq: Equation, current_formula: Formula,
                                                  fresh_variable_counter: int) -> Tuple[Equation, Formula, int, str]:
    '''
    Equation: [Terms] V1 = [Terms] a
    Assume V1 = V1'a
...
def _two_variables_branch_1_suffix(eq: Equation, current_formula: Formula, fresh_variable_counter: int) -> Tuple[
    Equation, Formula, int, str]:
    '''
...
def _two_variables_branch_2_prefix(eq: Equation, current_formula: Formula, fresh_variable_counter: int) -> Tuple[
    ...
    '''
    return _two_variables_branch_1_prefix(Equation(eq.right_terms, eq.left_terms), current_formula,
                                          fresh_variable_counter)
...
def _two_variables_branch_2_suffix(eq: Equation, current_formula: Formula, fresh_variable_counter: int) -> Tuple[
    ...
    '''
    return _two_variables_branch_1_suffix(Equation(eq.right_terms, eq.left_terms), current_formula,
                                          fresh_variable_counter)
...
def _two_variables_branch_3_suffix(eq: Equation, current_formula: Formula, fresh_variable_counter: int) -> Tuple[
    Equation, Formula, int, str]:
    '''
    Equation: [Terms] V1 = [Terms] V2
    Assume |V1| = |V2|
...
def _encode_non_linear_to_linear_formula(current_eq: Equation, rest_formula: Formula, fresh_variable_counter: int) -> \
    ...
    def _get_one_side(one_side_term_list, rest_formula, fresh_variable_counter):
        new_term_list = []
        for term in one_side_term_list:
            if term.value_type == Variable:
                if term.get_value_str in empty_variable_set:
                    fresh_variable_term, fresh_variable_counter = _create_fresh_variables(fresh_variable_counter)
                    new_term_list.append(fresh_variable_term)
                    rest_formula = Formula([Equation([term], [
                        fresh_variable_term])] + rest_formula.eq_list)  # add new equation to rest_formula
                else:
                    new_term_list.append(term)
                    empty_variable_set.add(term.get_value_str)
            else:  # terminal
                new_term_list.append(term)
        return new_term_list, rest_formula, fresh_variable_counter

    new_left_terms, rest_formula, fresh_variable_counter = _get_one_side(current_eq.left_terms, rest_formula,
                                                                         fresh_variable_counter)
    new_right_terms, rest_formula, fresh_variable_counter = _get_one_side(current_eq.right_terms, rest_formula,
                                                                          fresh_variable_counter)
