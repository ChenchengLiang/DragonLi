...
def main():
    ...
    benchmark_name = benchmark #"unsatcores_Benchmark_D_max_replace_length_bounded_16_train_20001_60000_onecore"
    for fo in os.listdir(bench_folder + "/" + benchmark_name):
        folder_basename=os.path.basename(fo)
        if "divided" in folder_basename:
            process_one_folder(benchmark_name, folder_basename)

        if "valid_data" in folder_basename:
            process_one_folder(benchmark_name, folder_basename)
...
def process_one_folder(benchmark_name,folder):
