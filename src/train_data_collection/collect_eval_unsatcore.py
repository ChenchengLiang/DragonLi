...
def main():
    ...
    for folder in folder_list:
        ...
        for eq_file in file_list:
            ...
            if os.path.exists(predicted_unsatcore_folder) == True and os.path.exists(unsatcore_folder) == True:
                predicted_eq_list=parser.parse(f"{folder}/{striped_base_file_name}.predicted_unsatcore")["equation_list"]
                predicted_unsatcore_eq_list=parser.parse(f"{predicted_unsatcore_folder}/{striped_base_file_name}.current_unsatcore")["equation_list"]
                ...
                unsatcore_eq_list=parser.parse(f"{unsatcore_folder}/{striped_base_file_name}.current_unsatcore")["equation_list"]
                #read summary json file
    ...
    predicted_unsatcore_size_unsatcore_size_diff_list = [predicted_unsatcore_size_list[i]-unsatcore_size_list[i] for i in range(len(original_problem_size_list))]
    unsatcore_size_predicted_unsatcore_size_ratio = sum(unsatcore_size_list)/sum(predicted_unsatcore_size_list)
    unsatcore_size_predicted_unsatcore_size_ratio_list = [unsatcore_size_list[i]/predicted_unsatcore_size_list[i] for i in range(len(original_problem_size_list))]
    unsatcore_size_original_problem_size_ratio = sum(unsatcore_size_list)/sum(original_problem_size_list)
    unsatcore_size_original_problem_size_ratio_list = [unsatcore_size_list[i]/original_problem_size_list[i] for i in range(len(original_problem_size_list))]
    predicted_unsatcore_size_original_problem_size_ratio = sum(predicted_unsatcore_size_list)/sum(original_problem_size_list)
    predicted_unsatcore_size_original_problem_size_ratio_list = [predicted_unsatcore_size_list[i]/original_problem_size_list[i] for i in range(len(original_problem_size_list))]



    summary_dict = {"original_problem_size_min": min(original_problem_size_list),
                    ...
                    "predicted_unsatcore_unsatcore_size_size_diff_mean": statistics.mean(predicted_unsatcore_size_unsatcore_size_diff_list),
                    "predicted_unsatcore_unsatcore_size_size_diff_stdev": custom_stdev(predicted_unsatcore_size_unsatcore_size_diff_list),
