...
def write_to_csv(consistance_list,folder):
...
def run_solvers(file_path, algorithm_configuration_list, other_solver_list,log=False):
    #for sh_file in glob.glob(bench_folder + "/src/process_benchmarks/temp_shell/*"):
    for sh_file in glob.glob("src/process_benchmarks/temp_shell/*"):
        os.remove(sh_file)
    ...
    # other solvers
    for solver in other_solver_list:
        if solver == "woorpje":
